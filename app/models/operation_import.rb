...
class OperationImport
  ...
  def build_from_fcsv_row(row)
    ret = Hash[row.to_hash.map { |k, v| [k.underscore.tr(' ', '_'), force_utf8(v)] }].delete_if { |k, _v| !klass.column_names.include?(k) }
    ...
    ret[:contact_id] = Contact.where(:id => $1).first.try(:id) if row['contact'].to_s.match(/^\#(\d+):/)
