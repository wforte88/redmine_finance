...
class OperationRelationsControllerTest < ActionController::TestCase
  ...
  def test_should_post_create
    ...
    assert_difference 'OperationRelation.count', 1 do
      compatible_request :post, :create, :operation_id => 1, :relation => { :destination_id => 2 }
    end
  ...
  def test_should_post_create_xhr
    @request.session[:user_id] = 1
    assert_difference 'OperationRelation.count', 1 do
      compatible_xhr_request :post, :create, :operation_id => 1, :relation => { :destination_id => 2 }
    end
  ...
  def test_should_destroy
    ...
    assert_difference 'OperationRelation.count', -1 do
      compatible_request :delete, :destroy, :id => operation_relation.id
    end
  ...
  def test_should_destroy_xhr
    ...
    operation_relation = OperationRelation.create(:source_id => 1, :destination_id => 2, :relation_type => '0')
    assert_difference 'OperationRelation.count', -1 do
      compatible_xhr_request :delete, :destroy, :id => operation_relation.id
    end
    assert_response :success
  end
end
