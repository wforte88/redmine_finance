...
class AccountsControllerTest < ActionController::TestCase
  ...
  def test_should_put_update_with_custom_field
    cf = AccountCustomField.create(name: 'Account cf', field_format: 'string')
    @request.session[:user_id] = 1
    compatible_request :put, :update, :id => 1, :account => { :name => 'Changed account name', custom_field_values: { cf.id.to_s => 'CF value' } }
    assert_response :redirect
    assert_equal 'Changed account name', Account.find(1).name
    assert_equal 'CF value', Account.find(1).custom_field_values.last.value
  ensure
    cf.destroy
  end
